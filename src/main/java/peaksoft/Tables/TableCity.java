...
//    }
    ...
    public static void saveCities(String city_name, int mayorId) {
        String saveSQL = "insert into Cities(city_name, mayorId) values(?, ?);";
        try (Connection connection = JDBCConfig.JDBCConnection()) {
            PreparedStatement preparedStatement = connection.prepareStatement(saveSQL);
            preparedStatement.setString(1, city_name);
            preparedStatement.setInt(2, mayorId);
     ...
     public static List<City> getAllCities() {
        String getAllSQL = "select city_id, city_name, mayorId, mayor_id, " +
                "       mayor_age, cityid " +
